...
export const Horizontal: Story = {
  render: () => (
    <ScrollArea.Root className="w-96 rounded-md border whitespace-nowrap">
      <ScrollArea.Viewport>
        <div className="flex w-max space-x-4 p-4">
          {Array.from({ length: 20 }).map((_, i) => (
            ...
            <figure key={i} className="shrink-0">
              <div className="bg-muted flex h-36 w-64 items-center justify-center overflow-hidden rounded-md">
                <span className="text-muted-foreground text-sm">Artwork {i + 1}</span>
              </div>
        ...
        <div className="w-150 p-4">
          ...
          {Array.from({ length: 30 }).map((_, i) => (
            ...
            <div key={i} className="text-sm">
              This is a long line of text that extends beyond the scroll area width to demonstrate
