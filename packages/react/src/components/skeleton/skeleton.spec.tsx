...
describe("Skeleton", () => {
  ...
  describe("props forwarding", () => {
    ...
    test("forwards aria attributes", () => {
      ...
      const skeleton = container.querySelector('[data-slot="skeleton"]');
      expect(skeleton as HTMLElement).toHaveAttribute("aria-label", "Loading content");
    });

    test("forwards data attributes", () => {
      const { container } = render(<Skeleton data-testid="my-skeleton" />);
      const skeleton = container.querySelector('[data-slot="skeleton"]');
      expect(skeleton as HTMLElement).toHaveAttribute("data-testid", "my-skeleton");
    });

    test("applies inline style", () => {
      const { container } = render(<Skeleton style={{ height: "20px", width: "200px" }} />);
      const skeleton = container.querySelector('[data-slot="skeleton"]');
      expect(skeleton as HTMLElement).toBeInTheDocument();
    });
  });
